...
class Calc {
  ...
  double exponentiate(double firstNum, double secondNum) {
    // Math.pow returns a double, so use typecasting to account for large numbers

    double exponentProd = pow(firstNum, secondNum) as double;
  ...
  static void wholeThing() {
    ...
    String? firstInput = stdin.readLineSync(encoding: Encoding.getByName('utf-8')!);
    ...
    switch (operator) {
      ...
      case "*":
        ...
        break;
      case "**":
        print("${calc1.num1} raised to the power of ${calc1.num2} equals = ${calc1.exponentiate(firstNum, secondNum)}");
        backToMainMenu();
        break;
      case "/":
    if (firstNum == 0 || secondNum == 0) {
      print("""\nDivision by zero is not possible. 
      \nWhen dividing any number by zero, the result is undefined. 
      \nZero divided by any number is always zero. 
      \nDivision by zero violates mathematical rules and cannot be performed.""");
    }   else
        {print("${calc1.num1} divided by ${calc1.num2} equals = ${calc1.divide(firstNum, secondNum)}");}
        backToMainMenu();
        break;
      case "%":
        print("${calc1.num1} modulo ${calc1.num2} equals = ${calc1.modulus(firstNum, secondNum)}");
        backToMainMenu();
        break;
      case "exit":
        exit(0);
